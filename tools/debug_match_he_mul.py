...
def main():
    ...
    parser.add_argument("--metadata-csv",    required=True, help="Path to channel metadata CSV")
    ...
    parser.add_argument("--save-png",       default=None, metavar="PATH",
    ...
    if meta.get("channels"):
        ...
        selected_indices = [name_to_idx[n.lower()] for n in selected_names
    ...
    if selected_indices:
        ...
        raw_list = [
            _read_channel_overview(mx_store, mx_ax, mx_h, mx_w, ds, c).astype(np.float32)
    ...
    print(f"  Multiplex: {mx_w}×{mx_h} px  mpp={mx_mpp}  overview: {mx_chw_w}×{mx_chw_h}  channels loaded={n_ch}")
    print(f"  Scale HE→MX: {scale:.4f}")

    # Warp or resize each channel into H&E overview space
    print(f"  {'Warping' if registered else 'Resizing'} {n_ch} channels to {W_he}×{H_he} ...")
    ...
    def blend(he: np.ndarray, mx_rgb: np.ndarray, a: float) -> np.ndarray:
        return (he.astype(np.float32) * (1 - a) +
                mx_rgb.astype(np.float32) * a).clip(0, 255).astype(np.uint8)
    ...
    ax_mx.set_title(f"MX ch{init_ch}: {ch_label(init_ch)}", color="white", fontsize=11, pad=6)
    ax_mx.set_xticks([]); ax_mx.set_yticks([])
    ax_mx.set_facecolor("black")

    im_ov = ax_ov.imshow(init_ov)
    ax_ov.set_title(f"Overlay  α={args.alpha:.2f}  ch{init_ch}: {ch_label(init_ch)}",
    ...
    slider_ch = mwidgets.Slider(
        ax_sl_ch, "Channel", 0, n_ch - 1,
        valinit=init_ch, valstep=1, color="#ff8800",
    )
    ...
    slider_alpha = mwidgets.Slider(
        ax_sl_al, "Alpha", 0.0, 1.0,
        valinit=args.alpha, color="#4499ff",
    )
    ...
    btn_prev.label.set_color("white"); btn_prev.label.set_fontsize(9)
    btn_next.label.set_color("white"); btn_next.label.set_fontsize(9)
    ...
    def _redraw(c: int, a: float) -> None:
        ...
        ax_ov.set_title(f"Overlay  α={a:.2f}  ch{c}: {ch_label(c)}",
                        color="white", fontsize=11, pad=6)
